...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nombre === '') {
        ...
        $stmt = $pdo->prepare("UPDATE categorias_material SET nombre = :nombre, descripcion = :descripcion WHERE id = :id");
        $stmt->execute([
            ':nombre' => $nombre,
            ':descripcion' => $descripcion,
            ':id' => $id
        ]);
        header("Location: categorias_material.php?mensaje=Categoría actualizada");
        exit;
    }
}
?>
